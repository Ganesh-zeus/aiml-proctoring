...
async function trainModel() {
  ...
  for (let i = 1; i <= imageCount; i++) {
    ...
    // img = await fetch("http://localhost:64000/FileManager?label"+IMAGE_LABEL,{ mode: 'no-cors'});
    // img = await faceapi.fetchImage("http://localhost:64000/data/GANESH/GANESH.jpg");
    // sfetch(`http://localhost:64000/FileManager/${IMAGE_LABEL}/`).then((data) => {console.log(data);});
    // uri = await fetch("http://localhost:64000/FileManager?"+ new URLSearchParams({label:IMAGE_LABEL}));

    // img = await faceapi.fetchImage(`http://localhost:4200/assets/ai-library/${IMAGE_LABEL}/${IMAGE_LABEL}.jpg`);

    img = await faceapi.fetchImage("http://localhost:4200/assets/ai-library/GANESH.jpg");
